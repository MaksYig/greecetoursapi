...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(new AppError('Your are not loggedin.Please login to get access', 401));
  }
  ...
  if (!currentUser) {
    return next(new AppError('Thse user belonging to this token no longer exist', 401));
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email: req.body.email });
  if (!user) {
    return next(new AppError('There is not user with that Email adress 😕', 404));
  }
